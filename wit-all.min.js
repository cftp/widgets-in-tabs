...
/*! Wigets in Tabs */
(function(a){a.fn.wit=function(){return this.each(function(){var h=a(this);var e={interval:0,string_untitled:"Untitled"};
if(typeof WIT_OPTIONS=="object"&&WIT_OPTIONS!==null){e.interval=parseInt(WIT_OPTIONS.interval,10);e.string_untitled=WIT_OPTIONS.string_untitled;}var c=h.find(".wit-tab-title");
var g=h.find(":header").first().addClass("wit-title");g.text("");c.each(function(){var k=a(this);var i=a.trim(k.text());var j=k.parent().attr("id");k.remove();
if(""===i){i=e.string_untitled;}g.append(a("<span/>",{href:"#"+j,text:i,"class":"wit-tab-title"}));});var b=h.find(".wit-tab-content");b.each(function(i,j){if(g.children('span[href="#'+j.id+'"]').length==0){var k=a("<span/>",{href:"#"+j.id,text:e.string_untitled,"class":"wit-tab-title"});
if(i>=g.children().size()){g.append(k);}else{g.children().eq(i).before(k);}}});c=g.children();h.on("click","span.wit-tab-title",function(j){j.preventDefault();
var i=a(this);c.filter(".wit-selected").removeClass("wit-selected");i.addClass("wit-selected");b.filter(":visible").slideUp();b.filter(i.attr("href")).slideDown();
f(i);});function f(l){var m=g.scrollLeft();var n=g.offset().left;var k=n+g.outerWidth();var j=l.offset().left;var i=j+l.outerWidth();if(i>k){g.animate({scrollLeft:(m+(i-k))},400);
}else{if(j<n){g.animate({scrollLeft:(m-(n-j))},400);}}}c.first().addClass("wit-selected");b.hide();b.first().show();g.perfectScrollbar({wheelSpeed:5,wheelPropagation:false,useBothWheelAxes:true,useKeyboard:true,suppressScrollY:true});
function d(){var l=c.index(c.filter(".wit-selected"));var k=l+1;if(l>=c.length-1){l=c.length-1;k=0;}c.eq(l).removeClass("wit-selected");c.eq(k).addClass("wit-selected");
b.filter(":visible").slideUp();b.filter(c.eq(k).attr("href")).slideDown();f(c.eq(k));}if(e.interval!==0){setInterval(d,e.interval*1000);}});};})(jQuery);
